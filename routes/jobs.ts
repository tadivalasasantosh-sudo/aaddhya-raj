...
router.get('/public', async (req, res) => {
  try {
    if (mongoose.connection.readyState !== 1) {
      console.warn('MongoDB not connected, returning fallback jobs list');
      return res.json(fallbackJobs);
    }
    
    // Force re-seed to ensure new jobs are displayed
    await Job.deleteMany({});
    await Job.insertMany(fallbackJobs.map(({ _id, ...rest }) => rest));
...
router.post('/', isAuthorized, async (req, res) => {
  try {
    const { title, location, experience, description, skills, active } = req.body;
    const job = new Job({
      title,
      location,
      experience,
